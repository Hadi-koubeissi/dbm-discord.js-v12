...
mod: function(DBM) {
	...
	DBM.LeonZ.onStream = function(packet) {
		...
		if (packet.t == "VOICE_STATE_UPDATE" && packet.d.self_stream && !!packet.d.channel_id) {
			const server = Bot.bot.guilds.cache.get(packet.d.guild_id);
			const channel = server.channels.cache.get(packet.d.channel_id);
			const member = server.members.cache.get(packet.d.member.user.id);
