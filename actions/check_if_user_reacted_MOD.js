    ...
    action: function (cache) {
        ...
        if (member && reaction.users) {
            const member22 = String(member)
                .replace(/!/g, '');
            const ar = reaction.cache.users.array();
        ...
        if (reaction) {
            if (Array.isArray(member)) {
                result = member.every(function (mem) {
                    if (mem && reaction.cache.users) {
                        const member2 = String(mem)
                            .replace(/!/g, '');
                        const ar1 = reaction.cache.users.array();
            ...
            } else if (member && reaction.cache.users) {
                const member22 = String(member)
                    .replace(/!/g, '');
                const ar2 = reaction.cache.users.array();
