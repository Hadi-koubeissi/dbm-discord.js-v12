module.exports = {
	...
	action: function(cache) {
		...
		switch(info) {
			case 0:
				result = find2 == 0 ? server.members.cache.get(find) : this.getDBM().Bot.bot.users.cache.get(find)
				break;
			case 1:
				result = find2 == 0 ? server.members.cache.find(mem => {
					return mem.user ? mem.user.username === find : false;
				}) : this.getDBM().Bot.bot.users.cache.find(mem => {
					return mem ? mem.username === find : false;
				})
				break;
			case 2:
			result = find2 == 0 ? server.members.cache.find(element => element.displayName === find) : this.getDBM().Bot.bot.users.cache.find(element => element.displayName === find)
				break;
			case 3:
			result = find2 == 0 ? server.members.cache.find(mem => {
				...
				break;
			case 4:
			result = find2 == 0 ? server.members.cache.find(element => element.displayColor === find) : this.getDBM().Bot.bot.users.cache.find(element => element.displayColor === find)
				break;
			default:
				break;
		}
