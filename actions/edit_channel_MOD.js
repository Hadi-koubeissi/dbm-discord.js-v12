module.exports = {
	...
	action: function(cache) {
		...
		if(toChange === 1) {
			channel.setTopic(newState);
		} else if(toChange == 0) {
			channel.setName(newState);
		} else if(toChange == 2) {
			channel.setPosition(newState);
		} else if(toChange == 3) {
			channel.setBitrate(parseInt(newState));
		} else if(toChange == 4) {
			channel.setUserLimit(parseInt(newState));
		} else if(toChange == 5) {
			channel.setParent(newState);
		} else if(toChange == 6) {
			channel.setRateLimitPerUser(newState);
		} else if(toChange == 7) {
			channel.setNFSW(Boolean(neState == "true"));
		}
