module.exports = {
	...
	<div>
    	<p>
        	<u>Mod Info:</u><br>
			Created by MrGold improve by LeonZ<br>
	...
	action: function(cache) {
		...
		var fetch = WrexMODS.require('node-fetch');
  	...
  	if (!fs.existsSync(filePath)){
		...
		fetch(url).then(res => {
			res.body.pipe(fs.createWriteStream(path.resolve(filePath, fileName + '.' + fileFormat)))
			stream.on('error', (err) => {
