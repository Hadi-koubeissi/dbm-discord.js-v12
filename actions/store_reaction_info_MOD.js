...
action: function(cache) {
	...
	switch(info) {
		...
		case 1:
			...
			break;
		case 2:
			result = rea.users.cache.array();
			break;
		case 3:
			result = rea.emoji.name;
			break;
		case 4:
			result = rea.count;
			break;
		case 5:
			const firstid = rea.users.cache.firstKey();
			result = cache.server.members.cache.get(firstid);
			break;
		case 6:
			const randomid = rea.users.cache.randomKey();
			result = cache.server.members.cache.get(randomid);
			break;
		case 7:
			const lastid = rea.users.cache.lastKey();
