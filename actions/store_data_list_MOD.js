	...
	<div style="width: 550px; height: 350px; overflow-y: scroll;">
		...
		<div>
			...
			<div id="Input2" style="display: null; padding-left: 5%; float: left; width: 65%;">
				...
				<input id="resultFormat" class="round" type="text" placeholder="Object + 'DataName' + DataValue"><br>
			</div>
		</div><br>
		<div>
		...
		switch (resultInfo) {
			case 0:
				...
				for (; resultFrom < resultTo; resultFrom++) {
					if (File == 0) {
						if (serverType == 0) {
							Object = cache.msg.guild.members.get(result[resultFrom].id);
						} else {
							Object = this.getDBM().Bot.bot.users.get(result[resultFrom].id);
						}
					} else {
						Object = result[resultFrom].name;
					}
