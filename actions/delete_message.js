module.exports = {
	...
	subtitle: function(data) {
		const names = [
			'Command Message', 'Temp Variable', 'Server Variable', 'Global Variable'];
	...
	action: function(cache) {
		...
		const options = {};
		if (data.reason) {
			options.reason = this.evalMessage(data.reason, cache);
		};
		if(Array.isArray(message)) {
			this.callListFunc(message, 'delete', [options]).then(function() {
				this.callNextAction(cache);
			}.bind(this));
		} else if(message && message.delete) {
			message.delete(options).then(function() {
				this.callNextAction(cache);
